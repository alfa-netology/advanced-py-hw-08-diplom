...
class Bot:
    ...
    def start(self):
        for event in self.longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text:
                ...
                self.send_message(sender_id=event.user_id, **message)
    ...
    def response_manager(self, event):
        ...
        if received_message in ('начать', 'привет', 'зравствуй', 'здоров', 'старт', 'погнали', 'hi'):
            ...
            return dict(message='А вот и кнопочки', keyboard=self.show_keyboard())

        elif received_message == 'спрячь клавиатуру':
            message = 'Клавитура убрана.\n' \
                      'Помни про команду Клавиши.'
            return dict(message=message, keyboard=self.hide_keyboard())
            ...
            return dict(message='Держи картиночку', attachments=attachments)
